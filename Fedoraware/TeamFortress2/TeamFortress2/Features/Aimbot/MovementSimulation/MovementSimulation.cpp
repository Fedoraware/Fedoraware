...
public:
    inline void Store(CBaseEntity* pPlayer)
    {
    ...
    inline void Restore(CBaseEntity* pPlayer)
    {
...
void CMovementSimulation::RunTick(CMoveData& moveDataOut, Vec3& worldSpaceCenterOut)
{
    ...
    reinterpret_cast<void(__thiscall*)(void*, CBaseEntity*, CMoveData*)>(Utils::GetVFuncPtr(g_Interfaces.CTFGameMovement, 1))(g_Interfaces.CTFGameMovement, m_pPlayer, &m_MoveData);
    g_GlobalInfo.predFutureLines.push_back(m_MoveData.m_vecAbsOrigin);
    moveDataOut = m_MoveData;
    Vec3 vMin, vMax; m_pPlayer->GetRenderBounds(vMin, vMax);
