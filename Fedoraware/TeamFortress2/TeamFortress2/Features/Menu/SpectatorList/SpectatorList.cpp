...
void CSpectatorList::DrawDefault()
{
	...
	if (const auto& pLocal = g_EntityCache.GetLocal())
	{
		...
		const int nFontTall = g_Draw.m_Fonts[FONT_MENU].nTall;
...
void CSpectatorList::DrawClassic()
{
	if (const auto& pLocal = g_EntityCache.GetLocal())
	{
		...
		const int addyy = g_Draw.m_Fonts[FONT_ESP_NAME].nTall;
		...
		for (const auto& Spectator : Spectators)
		{
			...
			int w, h;
			I::VGuiSurface->GetTextSize(g_Draw.m_Fonts[FONT_ESP_NAME].dwFont,
										(Spectator.Mode + Spectator.Name).c_str(), w, h);

			const int nAddY = g_Draw.m_Fonts[FONT_ESP_NAME].nTall;
