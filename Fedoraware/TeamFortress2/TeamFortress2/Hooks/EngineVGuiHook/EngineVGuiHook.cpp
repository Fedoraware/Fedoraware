				...
				if (Vars::Misc::CL_Move::Enabled.m_Var)
				{
					...
					if (pLocal && pWeapon)
					{
						if (pLocal->GetLifeState() == LIFE_ALIVE)
						{
							...
							else if (Vars::Misc::CL_Move::DTBarStyle.m_Var == 2) { // literally directly pasted from deathpole and not tested so PLEASE M-FED TEST THIS
