...
int BulletDangerValue(CBaseEntity* pPatient)
{
	...
	for (const auto& player : g_EntityCache.GetGroup(EGroupType::PLAYERS_ENEMIES))
	{
		...
		if (HAS_CONDITION(player, TF_COND_PHASE))
		{
			return false;
		}
		...
		if (HAS_CONDITION(player, TF_COND_ZOOMED))
		{
...
int FireDangerValue(CBaseEntity* pPatient)
{
	...
	for (const auto& player : g_EntityCache.GetGroup(EGroupType::PLAYERS_ENEMIES))
	{
		...
		{
			if (HAS_CONDITION(pPatient, TF_COND_BURNING) && pPatient->GetHealth() < 250)
			{
				if (pPatient->GetClassNum() == CLASS_PYRO) { return 1; }
				return 2;
			}

			if (HAS_CONDITION(player, TF_COND_CRITBOOSTED_RAGE_BUFF)) { return 2; }
			shouldSwitch = 1;
		}
	}
