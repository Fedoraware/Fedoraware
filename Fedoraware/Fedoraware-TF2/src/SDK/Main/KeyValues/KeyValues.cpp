...
bool CKeyValUtils::LoadFromBuffer(KeyValues* key_value, char const* resource_name, const char* buffer, void* file_system, const char* path_id)
{
	...
	static FN load_from_the_buffer = (FN)g_Pattern.Find("engine.dll", "55 8B EC 83 EC 38 53 8B 5D 0C");
...
KeyValues* CKeyValUtils::Initialize(KeyValues* key_value, char* name)
{
	...
	static FN initialize = (FN)(g_Pattern.Find("engine.dll", "FF 15 ? ? ? ? 83 C4 08 89 06 8B C6") - 0x42);
...
{
	using fn = KeyValues * (__thiscall*)(KeyValues*, char*);
	static fn FN = reinterpret_cast<fn>(g_Pattern.Find("engine.dll", "FF 15 ? ? ? ? 83 C4 08 89 06 8B C6") - 0x42);
	FN(this, name);
}
